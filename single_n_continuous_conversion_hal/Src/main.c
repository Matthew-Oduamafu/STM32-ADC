...
static void MX_ADC1_Cont_Init(void);
/* USER CODE BEGIN PFP */
...
int main(void)
{
  ...
  for(int i = 0; i<8; ++i){
	  ...
	  while(HAL_OK != HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY)){} // wait for conversion to finish

	  uint32_t raw_value = HAL_ADC_GetValue(&hadc1); // get value

	  HAL_ADC_Stop(&hadc1); // Stop conversion

	  uint8_t light_level = adc_to_light_percent(raw_value);
